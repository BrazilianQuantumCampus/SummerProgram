...
def demonstrate_state_space_proof():
    ...
    # Porta Hadamard (Matriz)
    H_gate = (1/np.sqrt(2)) * np.array([[1, 1], [1, -1]])
    print(f"\nMatriz Hadamard (H):\n{H_gate}")
    
    # --- Parte 2: Cálculo Manual ---
    print("\n[Passo 2] Cálculo Manual (Matriz x Vetor)")
    # H * |0>
    ...
    # Adicionar medição explicitamente no próprio circuito para garantir visualização
    # qc.measure_all(inplace=True) # Removido a pedido do usuário para Postulado 1
    
    # Capturar a figura para adicionar título
